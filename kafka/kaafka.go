...
func InterviewScheduledMessage(email, company, date, designation string) error {
	config := sarama.NewConfig()
	config.Producer.RequiredAcks = sarama.WaitForAll
	config.Producer.Return.Successes = true
	config.Producer.Partitioner = NewFixedPartitioner()
	...
	msg := fmt.Sprintf(`{"Email": "%s", "Designation": "%s", "Date": "%s", "Company": "%s"}`, email, designation, date, company)
	message := []byte(msg)
	p, o, err := producer.SendMessage(&sarama.ProducerMessage{
		Topic:     topic,
		Value:     sarama.StringEncoder(message),
